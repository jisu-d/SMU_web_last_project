window.initCustomerService = function() {
    ...
    const itemsPerPage = 5; // 4에서 5로 증가하여 더 많은 항목 표시
    let currentPage = 1;
    ...
    if (searchInput) {
        searchInput.addEventListener('input', debounce(() => {
            // 검색 시 카테고리를 '전체'로 초기화하여 모든 FAQ 검색
            currentCategory = '전체';
            categoryButtons.forEach(btn => {
    ...
    if (targetFaqId) {
        ...
        if (targetItem) {
            ...
            // 3. 페이지 계산
            // 해당 항목이 카테고리 내에서 몇 번째인지 확인하여 페이지 찾기
